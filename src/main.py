...
from src.generators import ScriptGenerator, VoiceoverGenerator, VisualGenerator
from src.assembler import VideoAssembler
...
class VideoGenerationPipeline:
    ...
    def generate_video(
        ...
        """
        self.logger.info(
            f"Starting video generation for topic: {topic} {'(MOCK MODE)' if mock else ''}"
        )
        ...
        try:
            ...
            video_path = self.video_assembler.assemble(
                scenes=scenes, output_filename=output_filename, title=title
            )
    ...
    def _generate_voiceovers(self, scenes: list, mock: bool = False):
        ...
        for scene in tqdm(scenes, desc="Generating voiceovers"):
            ...
            try:
                audio_path, duration = self.voiceover_generator.generate_audio(
                    narration, mock=mock
                )
                scene["audio_path"] = (
                    audio_path if (not mock or Path(audio_path).exists()) else None
                )
                scene["duration"] = duration

            except Exception as e:
                self.logger.error(
                    f"Failed to generate voiceover for scene {scene.get('id')}: {e}"
                )
        ...
        for scene in tqdm(scenes, desc="Generating visuals"):
            ...
            try:
                ...
                enhanced_prompt = (
                    visual_prompt
                    if mock
                    else self.visual_generator.enhance_for_video(visual_prompt)
                )

                # Generate image
                image_path = self.visual_generator.generate_image(
                    enhanced_prompt, mock=mock
                )
                scene["image_path"] = (
                    image_path
                    if (not mock or Path(image_path).exists())
                    else "mock_image.png"
                )

            except Exception as e:
                self.logger.error(
                    f"Failed to generate visual for scene {scene.get('id')}: {e}"
                )
    ...
    def validate_configuration(self) -> bool:
        ...
        if img_provider != "local":  # Local doesn't need API key
            img_key = (
                self.config.get("image_generation", {})
                .get(img_provider, {})
                .get("api_key")
            )
...
def main():
    ...
    parser.add_argument(
        "--mock", action="store_true", help="Run in mock mode without calling real APIs"
    )
    ...
    try:
        ...
        video_path = pipeline.generate_video(
            topic=args.topic, output_filename=args.output, mock=args.mock
        )
